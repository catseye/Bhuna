...
/*
	...
	switch (t->tclass) {
	case TYPE_LIST:
		type_free(&t->t.list.contents);
		break;
	case TYPE_DICT:
		type_free(&t->t.dict.index);
		type_free(&t->t.dict.contents);
		break;
	case TYPE_CLOSURE:
		type_free(&t->t.closure.domain);
		type_free(&t->t.closure.range);
		break;
	case TYPE_ARG:
		type_free(&t->t.arg.left);
		type_free(&t->t.arg.right);
		break;
	}
...
type_dup(struct type *t)
{
	...
	switch (t->tclass) {
	case TYPE_VAR:
		n->t.var.num = t->t.var.num;	unless...
		n->unifier = t->unifier;
		break;
	case TYPE_LIST:
		n->t.list.contents = type_dup(t->t.list.contents);
		break;
	case TYPE_DICT:
		n->t.dict.index = type_dup(t->t.dict.index);
		n->t.dict.contents = type_dup(t->t.dict.contents);
		break;
	case TYPE_CLOSURE:
		n->t.closure.domain = type_dup(t->t.closure.domain);
		n->t.closure.range = type_dup(t->t.closure.range);
	case TYPE_ARG:
		n->t.arg.left = type_dup(t->t.arg.left);
		n->t.arg.right = type_dup(t->t.arg.right);
	}
